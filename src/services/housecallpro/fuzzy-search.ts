/**
 * Customer identification algorithms for the HouseCall Pro integration.
 * Ported from the BuildOps fuzzy-search module (Jaro-Winkler, Soundex, USPS
 * address normalization, token-set ratio) and adapted to HCP row shapes:
 * customers carry first_name/last_name and a list of first-class addresses.
 *
 * Scenario A (customer lookup): computeMatchSignals + assignTier + crossValidate.
 * Scenario B (address match): addressSimilarityScore (see handlers/customer.ts).
 */

import type { HcpCustomerRow, FuzzyQuery, ScoredCandidate, LookupDecision } from './types.js';
...
function scoreCandidate(query: FuzzyQuery, customer: HcpCustomerRow): number {
  let score = 0;

  const customerFirst = normalizeName(customer.firstName ?? '');
  const customerLast = normalizeName(customer.lastName ?? '');
  ...
  // Score against the customer's (hydrated) addresses. Empty for cache-only rows,
  // in which case address/city/zip contribute nothing and matching is name-driven.
  const addrs = customer.addresses ?? [];
  const spokenAddress = query.address;

  if (spokenAddress) {
    const normalizedSpoken = normalizeAddress(spokenAddress);
    const addrScores = addrs
      .filter(a => a.street)
      .map(a => tokenSetRatio(normalizedSpoken, normalizeAddress(a.street!)));
  ...
  if (query.zip && firstAddr?.zip) {
    score += (query.zip.slice(0, 5) === firstAddr.zip.slice(0, 5) ? 1 : 0) * W_ZIP;
  }

  if (query.oldPhone) {
    const qPhone = normalizePhoneLast10(query.oldPhone);
    if (customer.allNumbers.some(p => p === qPhone)) score += W_PHONE;
  }
...
export interface MatchSignals {
  ...
  customersForName: number;
  customersForExactPhone: number;
  nameMatchStrong: boolean;
  nameMatchWeak: boolean;
  nameMismatch: boolean;
  queryHasFullName: boolean;
  queryHasFirstNameOnly: boolean;
}
...
export function computeMatchSignals(
  ...
  candidate: HcpCustomerRow,
  stats: { customersForName: number; customersForExactPhone: number },
): MatchSignals {
  ...
  const nameMatchStrong = !!(
    (qNorm && qNorm === cNorm) ||
    (lastExact && firstFuzzy >= 0.75) ||
    (lastFuzzy >= 0.85 && firstFuzzy >= 0.75) ||
    nameFuzzy >= 0.75
  );
  ...
  const addrStrings: string[] = (candidate.addresses ?? [])
    .filter(a => a.street)
    .map(a => [a.street, a.city, a.state, a.zip].filter(Boolean).join(' '));
  ...
  return {
    ...
    nameExact,
    nameFuzzy,
    namePrefixMatch: prefixMatch,
    addressSimilarity: bestAddrScore,
    addressQueryMatch: addrQMatch,
    addressMatch: addrMatch,
    customersForName: stats.customersForName,
    customersForExactPhone: stats.customersForExactPhone,
    nameMatchStrong,
    nameMatchWeak,
    nameMismatch,
    queryHasFullName,
...
export function assignTier(s: MatchSignals): TierAssignment {
  // Tier 1 — high confidence, auto-confirm
  if (s.phoneExact && s.addressMatch)
    return { tier: 1, rule: 'phone_and_address_match' };
  if (s.phoneExact && s.customersForExactPhone === 1)
    return { tier: 1, rule: 'phone_match_single_customer' };
  if (s.nameExact && s.addressMatch)
    return { tier: 1, rule: 'name_and_address_match' };
  ...
  if (s.nameExact && s.customersForName === 1)
    return { tier: 1, rule: 'name_exact_single_customer' };

  // Tier 2 — medium confidence, confirm verbally
  if (s.phoneExact && s.customersForExactPhone > 1)
    return { tier: 2, rule: 'phone_match_multiple_customers' };
  ...
  if (s.nameExact && s.addressMatch)
    return { tier: 2, rule: 'name_and_address_exact' };
  if (s.nameFuzzy > 0.9 && s.addressMatch)
    return { tier: 2, rule: 'name_fuzzy_and_address' };
  if (s.namePrefixMatch && s.addressMatch)
    return { tier: 2, rule: 'name_prefix_and_address' };
  if (s.nameExact)
    return { tier: 2, rule: 'name_exact' };
  if (s.nameFuzzy > 0.8 && s.addressMatch)
    return { tier: 2, rule: 'name_fuzzy_strong_and_address' };
  ...
  if (s.nameFuzzy > 0.7)
    return { tier: 3, rule: 'name_fuzzy_weak' };
  if (s.addressMatch)
...
export function crossValidate(
  ...
  if (signals.phoneExact && signals.customersForExactPhone > 1 && hasName && !nameOk) {
...
export function pickPrimaryAddress(
  ...
  const formatted = a.formatted || [a.street, a.city, a.state, a.zip].filter(Boolean).join(', ');
